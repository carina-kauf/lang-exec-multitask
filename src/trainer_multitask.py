...
def main(args, model_save_dir):
    ...
    for epoch in train_iterator:
        ...
        for step in range(epoch_length):
            ...
            weighted_loss = get_weighted_loss(losses) # calculate the weights for each task loss
            optimizer.zero_grad() #clear gradients
            weighted_loss.backward() #backpropagate the weighted loss
        ...
        for task in args.tasks:
            ...
            _logger.debug(f"{task} | Training epoch {epoch}: unweighted loss = {epoch_task_loss}, perplexity = {np.exp(epoch_task_loss)}")
