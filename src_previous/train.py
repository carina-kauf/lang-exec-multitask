...
def get_pretrained_emb(TASK2DIM, lang_data_dict):
    ...
    for key in TASK2DIM.keys():
        if "yang" in key:
            continue
        elif any(x in key for x in ["wikitext", "penntreebank", "pennchar"]):
...
def plot_loss(args, epoch, loss_data, model_save_dir):
    ...
    loss_plot_df_wide = loss_plot_df.pivot_table(index="step", columns="dataset", values="loss")
...
def main(args, model_save_dir):
    ...
    if args.glove_emb:
        try:
            assert any(x in task for task in args.tasks for x in ["wikitext", "de_wiki", "penntreebank", "pennchar" "contrib."])
